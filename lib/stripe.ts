...
export const STRIPE_PAYMENT_LINK_ID = "plink_1TAMQyDhkmzF1Lbv1hALtFao";

/** Full Stripe Payment Link URL for the Pro upgrade checkout page. */
export const STRIPE_PAYMENT_LINK_URL =
  "https://buy.stripe.com/test_7sYfZhg0J6nF45VdKA2ZO0d";
...
export async function checkProAccess(email: string): Promise<boolean> {
  try {
    ...
    // If the check fails (network error, timeout, etc.) default to free tier
    // so we never accidentally block a paying user — they can retry.
    return false;
  }
}
